...
class GNews:
    ...
    def _ceid(self):
        ...
        if self._start_date or self._end_date:
            if inspect.stack()[2][3] != "get_news":
                warnings.warn(
                    message=(
                        "Only searches using get_news support date ranges. "
                        "Start and end dates will be ignored."
                    ),
        ...
        return time_query + "&hl={}&gl={}&ceid={}:{}".format(
            self._language,
            self._country,
            self._country,
            self._language,
    ...
    def get_full_article(self, url):
        ...
        try:
            ...
            raise NetworkError(
                f"An error occurred while fetching the article: {error}"
            ) from error
    ...
    standard_output = (
        ...
        + "{'title': Article Title,"
        + indent
        + "'description': Google News summary of the "
        "article,"
        + indent
        ...
        + "'publisher':"
        + indent2
        + "{'href': link to publisher's website,"
        + indent2
                ...
                if article["url"] not in seen_urls:
                    ...
                    try:
                        published_date = datetime.datetime.strptime(
                            published_date, "%a, %d %b %Y %H:%M:%S GMT"
                        )
    ...
    def get_news_by_topic(self, topic: str):
        ...
        if topic in TOPICS:
            ...
            return self._get_news(query)
        raise InvalidConfigError(
            f"Invalid topic '{topic}'. Must be one of {list(TOPICS) + list(SECTIONS.keys())}."
        )
