        ...
        for item in root.findall(".//item"):
            ...
            if pub_date_elem is not None and pub_date_elem.text:
                try:
                    ...
                    pub_time = parsedate_to_datetime(pub_date_elem.text)
                    # Convert to UTC
...
def _fetch_itjuzi_rss():
    ...
    try:
        ...
        resp = httpx.get(url, headers=headers, timeout=15)
        if resp.status_code != 200:
            logger.warning(f"ITJuzi RSS returned {resp.status_code}")
            return []
        ...
        for item in root.findall(".//item"):
            ...
            # Parse pubDate - Format: 2026-02-02 15:48:11 (东八区时间)
            pub_time = None
            if pub_date_elem is not None and pub_date_elem.text:
                try:
                    ...
                    pub_time = datetime.strptime(
                        pub_date_elem.text.strip(), "%Y-%m-%d %H:%M:%S"
                    )
                    pub_time = pub_time.replace(tzinfo=ZoneInfo("Asia/Shanghai"))
                    # Convert to UTC
                    pub_time = pub_time.astimezone(timezone.utc)
            ...
            if desc_elem is not None and desc_elem.text:
                ...
                # Keep original content without HTML stripping
        ...
        logger.error(f"Error fetching ITJuzi RSS: {e}")
        return []
...
class ITJuziCollector(BaseCollector):
    """IT桔子创投快讯采集器 - 使用RSS feed"""
    ...
    async def fetch_articles(self) -> list[RawArticle]:
        try:
            loop = asyncio.get_running_loop()
            items = await loop.run_in_executor(_executor, _fetch_itjuzi_rss)
            ...
            logger.info(f"Fetched {len(articles)} articles from ITJuzi")
            return articles

        except Exception as e:
            logger.error(f"Error fetching from ITJuzi: {e}")
            return []
